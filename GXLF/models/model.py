...
from models.MGC import MGC
from models.xlsm import xLSTM
            ...
            if l % 2 == 0:
                encoder_layers.append(EncoderLayer(
                    AttentionLayerWin(Attn(False, factor, attention_dropout=dropout, output_attention=output_attention),
                                      d_model, n_heads, mix=False, output_attention=output_attention,
...
class FWinwithGCN(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len,
                 factor=5, d_model=512, n_heads=8, e_layers=3, d_layers=2, d_ff=512,
                 dropout=0.0, attn='prob', embed='fixed', freq='h', activation='gelu',
                 output_attention=False, distil=True, mix=True,
                 device=torch.device('cuda:0'), window_size=24, num_windows=4, dwindow_size=0):
        super(FWinwithGCN, self).__init__()
        self.pred_len = out_len
        self.attn = attn
        self.output_attention = output_attention
        if dwindow_size == 0:
        ...
        # ✅ 添加 MGC 模块第一步（放在 Encoder 前）
        self.mgc = MGC(in_dim=d_model, drop_path=0.0, K=2)
        # Attention
        # if attn == 'ds':
        #     Attn = DSAttention
        # elif attn == 'prob':
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # Step 1: Embedding
        enc_out = self.enc_embedding(x_enc, x_mark_enc)
        # ✅ Step 2: MGC before Encoder第二步
        enc_out = enc_out.permute(0, 2, 1).unsqueeze(-1)  # [B, D, T, 1]
        # print("Before MGC shape:", enc_out.shape)
        enc_out = self.mgc(enc_out)  # [B, D, T, 1]
        # print("After MGC shape:", enc_out.shape)
        enc_out = enc_out.squeeze(-1).permute(0, 2, 1)  # [B, T, D]
        # Step 3: Encoder
        enc_out, attns = self.encoder(enc_out, attn_mask=enc_self_mask)
        # Step 4: Decoder
        dec_out = self.dec_embedding(x_dec, x_mark_dec)
...
class FWinwithGCNXLSTM(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len,
                 factor=5, d_model=512, n_heads=8, e_layers=3, d_layers=2, d_ff=512,
                 dropout=0.0, attn='prob', embed='fixed', freq='h', activation='gelu',
                 output_attention=False, distil=True, mix=True,
                 device=torch.device('cuda:0'), window_size=24, num_windows=4, dwindow_size=0):
        super(FWinwithGCNXLSTM, self).__init__()
        self.pred_len = out_len
        self.attn = attn
        self.output_attention = output_attention
        ...
        # ✅ 添加 xLSTM 模块（在 MGC 之后）
        self.xlstm = xLSTM(d_model, d_model, 3)  #4是num_heads，即“多头并行结构”的头数。模型定义了ms 都是一个
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # ✅ Step 3: xLSTM
        enc_out = self.xlstm(enc_out)                     # [B, T, D]
